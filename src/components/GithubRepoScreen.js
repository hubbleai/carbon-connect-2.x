...
function GithubRepoScreen({ username, activeIntegrations, setPauseDataSourceSelection }) {
	...
	useEffect(() => {
		const ghSource = activeIntegrations
			.find(i => { return i.data_source_type == 'GITHUB' && i.data_source_metadata?.['username'] == username })
	...
	const syncRepos = async () => {
		if (!dataSource) {
			toast.error("Unable to find a connected data source")
			return
		}
		try {
			const res = await authenticatedFetch(
						...
						<div className="cc-flex cc-w-full cc-grow cc-flex-col cc-pb-8">
							<div className="cc-flex cc-flex-row cc-items-center cc-justify-between">
								<div className="cc-flex cc-items-center cc-space-x-2 cc-text-sm cc-px-2">
									Select the repositories you want to view files from
								</div>
								...
								<div className="cc-flex cc-flex-row cc-space-x-2">
									<button
										...
										onClick={syncRepos}
									>
										Select {selectedRepos.length} Repos
									</button>
									<button
										className={`cc-justify-end ${selectedRepos.length == 0 ? 'cc-hidden' : ''
